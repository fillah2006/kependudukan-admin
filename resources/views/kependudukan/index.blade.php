...
@section('content')
    <div class="container mb-5">
            ...
            <form action="{{ route('penduduk.index') }}" method="GET" class="row g-2">
                ...
                <div class="col-md-6">
                    <input type="text" name="search" class="form-control neo-input"
                        placeholder="Cari nama, NIK, atau nomor telepon..." value="{{ request('search') }}">
                </div>
        ...
        <div class="row mb-4">
            ...
            <div class="col-md-3 mb-3">
                <div class="stats-card">
                    <div class="stats-number">{{ $maleCount ?? 0 }}</div>
                    <div class="stats-label">Laki-laki</div>
                    @if ($penduduks->total() > 0)
                        <div class="stats-percentage">
                            {{ number_format((($maleCount ?? 0) / $penduduks->total()) * 100, 1) }}%
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="col-md-3 mb-3">
                <div class="stats-card">
                    <div class="stats-number">{{ $femaleCount ?? 0 }}</div>
                    <div class="stats-label">Perempuan</div>
                    @if ($penduduks->total() > 0)
                        <div class="stats-percentage">
                            {{ number_format((($femaleCount ?? 0) / $penduduks->total()) * 100, 1) }}%
                        </div>
                    @endif
                </div>
            </div>
                            ...
                            <tbody>
                                @foreach ($penduduks as $penduduk)
                                    <tr>
                                        ...
                                        <td>
                                            <span
                                                class="badge-gender {{ $penduduk->gender == 'L' ? 'badge-male' : 'badge-female' }}">
                                                {{ $penduduk->gender == 'L' ? 'Laki-laki' : 'Perempuan' }}
                                            </span>
                                        </td>
                                        <td>
                                            <!-- STATUS -->
                                            {!! $penduduk->statusBadge !!}
                                        </td>
                                        <td>
                                            @if ($penduduk->nik)
                                        ...
                                        <td>
                                            <div class="action-buttons">
                                                ...
                                                @if ($penduduk->isMeninggal())
                                                    <a href="{{ route('kematian.show', $penduduk->kematian->id) }}"
                                                        class="btn btn-neo-info btn-sm" title="Detail Kematian">
                                                        <i class="fas fa-info-circle"></i>
                                                    </a>
                                                @else
                                                    <a href="{{ route('kematian.create') }}?penduduk_id={{ $penduduk->id }}"
                                                        class="btn btn-neo-danger btn-sm" title="Catat Kematian">
                                                        <i class="fas fa-plus-circle"></i> Kematian
                                                    </a>
                                                @endif
                            ...
                            <nav aria-label="Navigasi data penduduk">
                                <ul class="pagination pagination-neo mb-0">
                                    ...
                                    @else
                                        <li class="page-item">
                                            <a class="page-link"
                                                href="{{ $penduduks->withQueryString()->previousPageUrl() }}"
                                                rel="prev">
                                                <i class="fas fa-chevron-left"></i>
                                            </a>
                                        </li>
                                    ...
                                    @foreach ($penduduks->getUrlRange(1, $penduduks->lastPage()) as $page => $url)
                                        ...
                                        @else
                                            <li class="page-item">
                                                <a class="page-link"
                                                    href="{{ $penduduks->withQueryString()->url($page) }}">{{ $page }}</a>
                                            </li>
                                        @endif
                                    @endforeach
                                    ...
                                    @if ($penduduks->hasMorePages())
                                        <li class="page-item">
                                            <a class="page-link" href="{{ $penduduks->withQueryString()->nextPageUrl() }}"
                                                rel="next">
